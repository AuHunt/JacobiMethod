...
int jacobi() {
    ...
    do {
        ...
        for( i = 0; i < n; i++ ) { x[i] = buf[i]; }
        k++;
    } while( !convergence(k) );

    // Below will print the results
    printf("The value of k (iter) is %i\nThe results are ", k);
    for(i = 0; i < n; i++) { printf("%f ", buf[i]); }
    return k;
}
